#include <esp_now.h>  // For ESP32 and ESP-NOW
#include <WiFi.h>
#include <esp_wifi.h>
...
void loop() {
    ...
    // Loop through a set of conditions for each of 20 pins
    for (int i = 0; i < 20; i++) {  
        ...
        if (millis() - startMillis[i] > 500) {
            if (timer[i] == ON) {
                ...
                if (clk[i] == 1 && rel[i] == 0) {                     //hold
                    //outgoingReadings.switchState = db(sw(i));   /* appropriate state function like fo, db, ch, dbl */
                    Serial.println(db(sw(i)));
                } else if (clk[i] == 1 && rel[i] == 1) {              //click
                    //outgoingReadings.switchState = fo(sw(i));   /* appropriate state function like fo, db, ch, dbl */
                    Serial.println(fo(sw(i)));
                } else if (clk[i] == 2 && rel[i] == 1) {              //click-hold
                    //outgoingReadings.switchState = ch(sw(i));   /* appropriate state function like fo, db, ch, dbl */
                    Serial.println(ch(sw(i)));
                } else if (clk[i] == 2 && rel[i] == 2) {              //dbl-click
                    //outgoingReadings.switchState = dbl(sw(i));   /* appropriate state function like fo, db, ch, dbl */
                    Serial.println(dbl(sw(i)));
                } 
                esp_err_t result = esp_now_send(peerMacAddress, (uint8_t *) &outgoingReadings, sizeof(outgoingReadings));
                if (result == ESP_OK) {
                    Serial.println("Sent with success");
