...
void loop() {
  ...
  if (dTemperature < setpoint) {
    digitalWrite(4, HIGH);
    Serial.println("I turned on the heat");
    client.publish("00000046/setpoint", "I turned on the heat");
  }
  if (dTemperature > setpoint+3) {
    digitalWrite(4, LOW);
    Serial.println("I turned off the heat");
    client.publish("00000046/setpoint", "I turned off the heat");
  }
}
