...
// onOff_array: onOff 0, 1: {NA, 1, 2, 3, 4, 5, 6};
// lux_array:  lux 0-100 {NA, lux1, lux2, lux3, lux4, lux5, lux6};
// temp_array:  {temp1, temp2, temp3, temp4, temp5, temp6};
int onOff_array[]  = {-99, 0, 0, 0, 0, 0, 0};
int lux_array[]    = {-99, 0, 0, 0, 0, 0, 0};
int temp_array[]   = {-99, 0, 0, 0, 0, 0, 0};


//int mom_pin_array[]= {-99,13,4,14,16,27,17,26,18,25,19};
int low_pins[]  = {-99, 13, 4,  27, 17, 25, 19};
int high_pins[] = {-99, 14, 16, 26, 18, 33, 23};
void t_write(int i) {
...
// +1 -1 or 0: lt_array:  NA, lux1, lux2, lux3, lux4, lux5, lux6, temp1, temp2, temp3, temp4, temp5, temp6 
...
void receive_controls_json(String topic, String msg) {
  ...
  //device_id/N N=1-6 should accept "on", "off", "dim", "brighten", "heat", "cool", "release"
  //NA, lux1, lux2, lux3, lux4, lux5, lux6, temp1, temp2, temp3, temp4, temp5, temp6
...
void setup() { 
  ...
  for (int i = 0; i<=6; i++) { 
    digitalWrite(low_pins[i], OUTPUT);  //Low temp, High temp, and AC pins
    digitalWrite(low_pins[i], HIGH);
    digitalWrite(high_pins[i], OUTPUT);  //Low temp, High temp, and AC pins
...
void loop() {
  ...
  for(int i=1;i<=6;i++) {  
    ...
    if(lt_array[i+6] == 1 && temp_array[i] < 255)  {   // like increasing the temp slider
        temp_array[i] += 5;
        if (temp_array[i] >= 255) {temp_array[i] = 255;}
        t_write(i);
    }
    if(lt_array[i+6] == -1 && temp_array[i] > 0){  // like decreasing the temp slider
        temp_array[i] -= 5;
