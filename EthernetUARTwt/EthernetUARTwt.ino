#include <ETH.h>          //For WT32-ETH01 and UART Comms
...
#define UART_RX 17  // Choose appropriate pin for RX
#define UART_TX 5   // Choose appropriate pin for TX, but not used here since only receiving
...
void setup() {
    Serial.begin(115200);
    SerialUART.begin(9600, SERIAL_8N1, UART_RX, -1);  // RX only, TX not needed
    delay(1000);
...
void loop() {
    ...
    while (SerialUART.available() >= 2) {  // Check if at least 2 bytes are available
        byte switchNumber = SerialUART.read();
        byte switchState = SerialUART.read();
        if (SerialUART.available() > 0) {  // Check for any extra bytes that might be noise
            SerialUART.flush();  // Discard any extra bytes
        }

        String state;
        client.publish("debug",("Received: Switch "+String(switchNumber)+" State byte "+String(switchState,HEX)).c_str());
        switch(switchState) {
          ...
          default: 
            state = "?"; 
            Serial.println("Received an unexpected state value");
            continue; // Skip publishing if state is unknown
        }
        publish_controls(String(switchNumber), state);
    }
    delay(50); // Add delay at the end of loop
}
