...
void publish_reporting_json() {
  ...
  state_json["vG"]        = "amp,0,40;watt,0,960";
  ...
  power_W = ina226.getBusPower() / 1000.0;
  ...
  client.publish(topic.c_str(), String(current_A).c_str());
  topic = String(device_id)+"/watt";
  client.publish(topic.c_str(), String(power_W).c_str());
...
void specific_connect() {
  ...
  topic = String(device_id)+"/"+String("watt");
  client.subscribe(topic.c_str());
