...
// 3 RECEIVE CONTROLS (to this exact device, from callback)
void receive_controls_json(String topic, String messageTemp) {
  if(messageTemp == "restart") {ESP.restart();}
}

// 4 SEND CONTROLS SEND CONTROLS (publish_controls only if controller module)
...
//5 CONNECT TO TOPICS (if board receives control commands)
